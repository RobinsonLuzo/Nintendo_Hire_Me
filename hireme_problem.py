# Nintendo Hire Me problem
# All of these arrays contain hexidecimal

# 512 byte array
confusion = []

# 32 bit array, with each item consisting of 32 bytes
diffusion = [0xf26cb481,0x16a5dc92,0x3c5ba924,0x79b65248,0x2fc64b18,0x615acd29,0xc3b59a42,0x976b2584,
0x6cf281b4,0xa51692dc,0x5b3c24a9,0xb6794852,0xc62f184b,0x5a6129cd,0xb5c3429a,0x6b978425,
0xb481f26c,0xdc9216a5,0xa9243c5b,0x524879b6,0x4b182fc6,0xcd29615a,0x9a42c3b5,0x2584976b,
0x81b46cf2,0x92dca516,0x24a95b3c,0x4852b679,0x184bc62f,0x29cd5a61,0x429ab5c3,0x84256b97]

# As defined in source this returns "Reverse me fast". This is commented out here.
# The solution array converted into appropriate format is supplied.
"""input_arr = [0x66,0xd5,0x4e,0x28,0x5f,0xff,0x6b,0x53,0xac,0x3b,0x34,0x14,0xb5,0x3c,0xb2,0xc6,
0xa4,0x85,0x1e,0x0d,0x86,0xc7,0x4f,0xba,0x75,0x5e,0xcb,0xc3,0x6e,0x48,0x79,0x8f]"""
# One solution that can be used
"""input_arr = [0x5d, 0xc5, 0xda, 0x80, 0x81, 0xad, 0xe, 0xe6, 0xda, 0x88, 0xd9, 0x67, 0x43, 0x63, 0x1c, 0x87,
0x8b, 0x72, 0x60, 0x24, 0x4d, 0xbc, 0xd9, 0xa7, 0xfa, 0x86, 0x19, 0x26, 0x1e, 0x69, 0x3, 0x6]"""

# Solution generated by solution.py
input_arr = [0x37, 0x55, 0xd6, 0xfa, 0xb2, 0x3e, 0x52, 0x59, 0xef, 0x95, 0x76, 0xba, 0x7, 0xbf, 0x34,
0xae, 0xc, 0x1e, 0x37, 0x9e, 0x28, 0x9, 0xd, 0xd7, 0x72, 0xfb, 0xc9, 0x7, 0x18, 0x7, 0x38, 0x38]



def Forward(inp, conf, diff):

    for i in range(256):
        d = []

        for j in range(32):
            d += [conf[inp[j]]] # [conf[inp[j]]] is essentially a s-box
            inp[j] = 0

        for j in range(32):
            for k in range(32): 
                # Shift right >>
                inp[j] ^= d[k] * ((diff[j] >> k) & 1)
    
    result = ''.join([chr(conf[inp[i * 2]] ^ conf[inp[i * 2 + 1] + 256]) for i in range(16)])
    
    print(result)
    return result


# ord() to get number of char
def main():
    target = "Hire me!!!!!!!!\x00"
    #target = "Live Overflow!!!\x00"

    if Forward(input_arr, confusion, diffusion) == target:
        #print(target)
        return True

    else:
        return False


main()
